import { createDidJwk, verifyVcJwt, signVpJwt } from "@did-vc-rbac/shared";
import { randomUUID } from "node:crypto";
import { decryptJson, encryptJson } from "../../security/crypto";
export async function createWallet(db, passphrase) {
...
export async function importWalletCredential(db, holderDid, passphrase, vcJwt) {
...
export async function createPresentation(db, input) {
    ...
    const vcPayload = decryptJson(JSON.parse(credential.vc_jwt_encrypted), input.passphrase);
