...
export function createAuthRequest(db, config, targetPath, requiredRole) {
...
export function issueSession(db, config, holderDid, role, permissions) {
...
export async function verifyDirectPost(db, config, input, requestMeta) {
    const authRequest = db.prepare("SELECT * FROM auth_requests WHERE state = ?").get(input.state);
    if (!authRequest)
        throw new Error("state mismatch");
    if (authRequest.used)
        throw new Error("replay attempt");
    ...
    if (!nonceValidation.ok)
        throw new Error(nonceValidation.reason);
    ...
    if (vpClaims.aud !== authRequest.client_id)
    ...
    const subject = vcClaims.vc.credentialSubject;
    return {
        holderDid,
...
export function recordFailure(db, reason, meta) {
    writeAudit(db, {
        eventType: "login",
        outcome: "failure",
